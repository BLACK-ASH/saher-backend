...
export const updateAttendance = async (req: Request, res: Response) => {
    ...
    let update: Types.ObjectId[] = [], failure: Types.ObjectId[] = [];
    ...
    const inCollection = await Participant.find({ _id: { $in: failure }, }).select("_id").lean();
    ...
    await Workshop.updateOne({ _id: workshop._id }, { $addToSet: { participants: { $each: update } } })
