...
export const getLatestNotificationController = async (req: Request, res: Response) => {
  ...
  if (countNotification === 0) {
    return res
      .status(200)
      .json({ message: 'There are no notification', count: 0, data: null, success: true });
  }
...
export const deleteNotificationController = async (req: Request, res: Response) => {
  ...
  return res
    .status(200)
    .json({ message: 'The notification has been deleted successfully', success: true, data: null });
};
